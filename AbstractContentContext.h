...
typedef std::vector<LineContent> TableContent;
...
public:
	...
	struct TableOptions
	{
		//textOptions决定表格内文字的字体类型、字体大小以及字体颜色
		TextOptions textOptions;

		//borderOptions决定表格边框中线条的宽度，线条的颜色
		GraphicOptions borderOptions;

		//tableContent是表格的输入内容，是一个二维的string数组
		...
		TableOptions(TextOptions inTextOptions, 
					GraphicOptions inBorderOptions=GraphicOptions(), 
					TableContent inTableContent,
					double inTableWidth,
					double inCharacterSpacing = 0，
					double inWordSpacing = 0,
					double inTextLeading = textOptions.fontSize,
					bool inIsCentered = true)
		{
			textOptions = inTextOptions;
			borderOptions = inBorderOptions;
			tableContent = inTableContent;
			tableWidth = inTableWidth;
			characterSpacing = inCharacterSpacing;
			wordSpacing = inWordSpacing;
			textLeading = inTextLeading;
			isCentered = inIsCentered;
		}
	...
	void DrawTable(double inX,double inY,const TableOptions& tableOptions);
